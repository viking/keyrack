...
class TestUtils < Test::Unit::TestCase
  ...
  def test_generate_rsa_key
    ...
    assert_equal([rsa, 'private key'], Keyrack::Utils.generate_rsa_key('secret'))
  end

  def test_generate_aes_key
    SecureRandom.expects(:base64).with(128).returns("x" * 172)
    result = Keyrack::Utils.generate_aes_key
  ...
  def test_open_rsa_key
    ...
    assert_equal(rsa, Keyrack::Utils.open_rsa_key(rsa_path, 'secret'))
  end
  ...
  def test_open_aes_data
    ...
    assert_equal(aes, Keyrack::Utils.open_aes_data(aes_path, rsa))
  end
end
