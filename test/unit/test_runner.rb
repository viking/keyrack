...
class TestRunner < Test::Unit::TestCase
  ...
  def test_console
    ...
    rsa = mock("rsa key")
    Keyrack::Utils.expects(:open_rsa_key).with(File.expand_path(rsa_path, keyrack_dir), 'secret').returns(rsa).in_sequence(seq)
    aes = {'key' => '12345', 'iv' => '54321'}
    Keyrack::Utils.expects(:open_aes_data).with(File.expand_path(aes_path, keyrack_dir), rsa).returns(aes).in_sequence(seq)
    store = mock('filesystem store')
    Keyrack::Store::Filesystem.expects(:new).with('path' => store_path).returns(store).in_sequence(seq)
    Keyrack::Database.expects(:new).with('12345', '54321', store).returns(@database).in_sequence(seq)
